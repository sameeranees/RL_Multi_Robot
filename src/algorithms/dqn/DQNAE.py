...
from deep_q_orig_memory import DeepQ
...
class Env(Node):
    ...
    def get_reward(target, collision, lin_vel, ang_vel, min_laser, repetitive_behavior, distance, theta):
        ...
        # distance_reward = (old_distance - distance) * 5
        # if abs(distance_reward) < 100:
        #     calc_reward += distance_reward

        #     # Additional reward for being closer to the target
        #     if distance < old_distance:
        #         calc_reward += min((1 / max(distance, 1e-6)) * 100, 10)  # Avoid division by zero and clip max reward

        # # Reward for reducing the angle to the target
        # if (old_theta < 0 and theta < 0) or (old_theta > 0 and theta > 0):
        #     angle_reward = (abs(old_theta) - abs(theta)) * 20
        #     if abs(angle_reward) < 100:
        #         calc_reward += angle_reward

        #     # Additional reward for having a small angle to the target
        #     if abs(theta) < 0.3:
        #         calc_reward += min((1 / max(abs(theta), 1e-6)) * 0.2, 1)  # Avoid division by zero and clip max reward

        # Update old values
        # old_distance = distance
        # old_theta = theta

        # Scale the reward to prevent too large values
        # scaled_reward = calc_reward * 0.2

        # Logging for debugging
        # env.get_logger().info(f"reward: {scaled_reward}, distance: {distance}, theta: {theta}, action: {action}")
    ...
    try:
        for epoch in range(current_epoch + 1, epochs + 1):
            if(performance_tracker.has_reached_threshold()):
                _observation = env.reset(True,True)
            _observation = env.reset(False,True)
            #_observation = np.array(observation)
            done = False
            cumulated_reward = 0
            episode_step = 0
            ...
            while not done and episode_step <= steps:
                ...
                fwd_action = deepQ.selectAction(qValues, explorationRate)
                action = [0, 0]
                if fwd_action == 0:
                    action[0] = 1
                    action[1] = 0
                ...
                deepQ.addMemory(_observation, fwd_action, reward, newObservation, done)

                if stepCounter >= learnStart and stepCounter % update_frequency == 0:
                    total_q_value,q_value_count,total_loss,loss_count,max_q_value = deepQ.learnOnMiniBatch(minibatch_size, useTargetNetwork=(stepCounter > updateTargetNetwork))
                    add_total_q_value += total_q_value
                    add_q_value_count += q_value_count
                    add_total_loss += total_loss
                    add_loss_count += loss_count
                    # add_max_q_value = max(max_q_value,add_max_q_value)
                    add_max_q_value = max_q_value
                ...
                if add_q_value_count > 0:
                    ...
                    writer.add_scalar("Av. Q", av_qvalue, epoch)

                writer.add_scalar("Max. Q", add_max_q_value, epoch)
                writer.add_scalar("Cumulative Reward", cumulated_reward, epoch)
                writer.add_scalar("Distance",distance,epoch)
                writer.add_scalar("Theta",theta,epoch)
                writer.add_scalar("Sucesses",successes,epoch)
